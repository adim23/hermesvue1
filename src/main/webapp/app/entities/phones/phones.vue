<template>
  <div>
    <h2 id="page-heading" data-cy="PhonesHeading">
      <span v-text="$t('hermesvue1App.phones.home.title')" id="phones-heading">Phones</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('hermesvue1App.phones.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PhonesCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('hermesvue1App.phones.home.createLabel')"> Create a new Phones </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && phones && phones.length === 0">
      <span v-text="$t('hermesvue1App.phones.home.notFound')">No phones found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('phone')">
              <span v-text="$t('hermesvue1App.phones.phone')">Phone</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'phone'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="$t('hermesvue1App.phones.description')">Description</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'description'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('favourite')">
              <span v-text="$t('hermesvue1App.phones.favourite')">Favourite</span>
            ...
            <th scope="row" v-on:click="changeOrder('kind.name')">
              <span v-text="$t('hermesvue1App.phones.kind')">Kind</span>
            ...
            <th scope="row" v-on:click="changeOrder('company.name')">
              <span v-text="$t('hermesvue1App.phones.company')">Company</span>
            ...
            <th scope="row" v-on:click="changeOrder('citizen.id')">
              <span v-text="$t('hermesvue1App.phones.citizen')">Citizen</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="hermesvue1App.phones.delete.question" data-cy="phonesDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-phones-heading" v-text="$t('hermesvue1App.phones.delete.question', { id: removeId })">
